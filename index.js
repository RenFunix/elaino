...
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./worker/lib/Functions/exif')
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('./worker/lib/Functions/myfunc')
const clui = require('clui')
const { Spinner } = clui
const prompt = require('prompt-sync')({sigint:true});
const figlet = require('figlet')
const crypto = require("crypto")
const { color, bgcolor } = require("./worker/lib/Functions/color")
...
async function __START() {
    ...
    const sock = sockConnect({
        ...
        browser: ['Elaina EsM','Safari','1.0.0'],
        auth: state,
        version
    })
...
console.log(color(`█████████████████████████\n█▄─▄▄▀█▄─▄▄─█▄─▄▄─█▄─▄▄─█\n██─██─██─▄█▀██─▄████─▄███\n▀▄▄▄▄▀▀▄▄▄▄▄▀▄▄▄▀▀▀▄▄▄▀▀▀`)) 
const users = [
		{Creator: 'RenFunix', Whatsapp: 6282148864989, NameBot: global.namebot, Versi: 7 }
		];
		console.table(users)


sock.browserDescription = ["ElainaESM", "Mac", "3.0.0"];
...
require("./worker/command")(sock, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
...
// Setting
	...
	sock.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
       ...
       if (options.asSticker || /webp/.test(mime)) {
        let { writeExif } = require('./worker/lib/Functions/exif')
    ...
    sock.getFile = async (PATH, save) => {
        ...
        filename = path.join(__filename, './worker/src/' + new Date * 1 + '.' + type.ext)
    ...
    sock.sendFile = async(jid, PATH, fileName, quoted = {}, options = {}) => {
        ...
        if (options.asDocument) type = 'document'
        if (options.asSticker || /webp/.test(mime)) {
            let { writeExif } = require('./worker/lib/Functions/sticker.js')
            let media = { mimetype: mime, data }
